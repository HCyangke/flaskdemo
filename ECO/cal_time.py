...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        TSNDataSet("/nfs/project/UCF101/", args.train_list, num_segments=args.num_segments,
    ...
    val_loader = torch.utils.data.DataLoader(
        TSNDataSet("/nfs/project/UCF101/", args.val_list, num_segments=args.num_segments,
                   new_length=data_length,
                   modality=args.modality,
                   image_tmpl=args.rgb_prefix+rgb_read_format if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+rgb_read_format,
                   random_shift=False,
...
def validate(val_loader, model, criterion, iter, logger=None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        #tmp=time.time()-tmp
        #print('forward time: ',tmp)
        #print('clips: ',output.size())
        #exit()
        loss = criterion(output, target_var)
    ...
    print('batch_time: ',batch_time.avg,'forward_time: ',forward_time.avg)
    return top1.avg
